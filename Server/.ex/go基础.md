...
2.make(...)：只用于 slice / map / channel 这种引用类型的“初始化”  初始化结构体+分配内存
...
B         uint8   //slice的cap
noverflow uint16
hash0     uint32    //hash种子，防止被hash，
buckets    unsafe.Pointer   // 
oldbuckets unsafe.Pointer  //扩容
nevacuate  uintptr
extra      *mapextra
}
...
1.三色标记： 从根变量开始，利用广度优先变量对所有有依赖路径的变量染色，最后没染到色的变量就是垃圾变量
          ...
          多核并发：GC程序和 应用程序并发所带来的问题是，应用程序会修改GC程序已经标记的变量，为了解决GC与goroutine的并发问题，go提出了弱三色一致性：黑色变量直接引用白色变量时，必须保证有一个灰色变量

            并发会带来的问题1（引用）：已经被扫描过的黑色变量（不会再处理），又直接引用了新的白色变量，这就导致了这个白色变量最后会被当做垃圾回收掉

 
            并发会带来的问题2（删除）：覆盖旧指针导致GC语义(STAB)错误 ,  X->Y   T1：启动GC ---------    T2： X->Y2   Y    T3：GC end

          如何实现：混合读写屏障（回调操作），相当于在GC阶段的写操作会被包装一层，把直接引用白色变量的黑色变量置灰。
                                                GC阶段的断开引用的操作都会


    并发正确                              并发正确
未提交读  已提交读  可重复读  串行化    -  弱三色一致性
      锁+MVCC                          混合写屏障
...
不允许操作：向已经关闭的channel，nil channel写数据， 向nil channel读数据
